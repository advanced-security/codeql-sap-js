...
**Prefer `ql-mcp` MCP server tools** over raw `codeql` CLI commands when available — they provide structured output, automatic logging, and better integration.
...
1. **Understand current behavior:**
   ```bash
   # Use mcp_ql-mcp_codeql_resolve_tests to discover tests, or:
...
3. **Run tests:**
   ```bash
   # ql-mcp (preferred): use mcp_ql-mcp_codeql_test_run
   codeql test run <test-directory>
   ```
...
5. **Accept if correct:**
   ```bash
   # ql-mcp (preferred): use mcp_ql-mcp_codeql_test_accept
...
- **NEVER** run `mcp_ql-mcp_codeql_test_accept` (or `codeql test accept`) without reviewing `.actual` files
- **NEVER** commit failing tests
- **NEVER** skip test validation
- **NEVER** assume framework behavior - validate with `ql-mcp` tools or CodeQL CLI
